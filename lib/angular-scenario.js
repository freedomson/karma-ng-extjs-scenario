...
angular.scenario.dsl('browser', function() {
  ...
  chain.getExt = function(cb) {
      return this.addFutureAction('window.ext', function($window, $document, done) {
        cb($window.Ext);
        done(null, $window.Ext);
      });
  };
...
angular.scenario.output('html', function(context, runner, model) {
  ...
  context.append(
    '<div id="header">' +
    '  <h1><span class="angular">EXT JS</span>: Scenario Test Runner</h1>' +
